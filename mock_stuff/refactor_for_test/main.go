...
func (c *PostsClient) FetchPosts(limit int) ([]Post, error) {
	url := fmt.Sprintf("%s?limit=%d", c.baseURL, limit) // Hard dependency
	resp, err := c.httpClient.Get(url) // Hard dependency
	if err != nil {
...
func main() {
	...
	client := NewPostsClient(http.DefaultClient, baseURL)

	posts, err := client.FetchPosts(3)
	if err != nil {
